...
public class WeatherMainView extends VerticalLayout {
    ...
    private Grid<Cloud> clouGrid = new Grid<>(Cloud.class);

    private transient WeatherAppController controller;

    public record Cloud(int elevation, String desc) {}

    @Autowired
    public WeatherMainView(WeatherAppController controller) {
        ...
        // configure grid
        clouGrid.addColumn(Cloud::elevation)
            .setWidth("100px")
            .setHeader("Elevation");
        clouGrid.addColumn(Cloud::desc)
            .setWidth("150px")
            .setHeader("Description");
        clouGrid.setWidth("250px");
        clouGrid.setHeight("250px");
    ...
    private Component buildCloudVisibilityView() {
        ...
        layout.add(clouGrid, visibility);
    ...
    private float computeInches(float milimeters) {
    ...
    private String convertWindDirection(Integer degrees) {
        ...
        if((degrees > 338 && degrees <= 360) || (degrees >= 0 && degrees < 23)) {
