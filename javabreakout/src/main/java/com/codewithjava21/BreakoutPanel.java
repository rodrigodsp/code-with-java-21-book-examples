...
public class BreakoutPanel extends JPanel implements Runnable {
    ...
    private static final long serialVersionUID = -7928723370L;

    private static final int fPS = 60; // frames per second
    private static final int brickWidth = 64;
    private static final int brickHeight = 32;
    private static final int brickBuffer = 128;
    private static final int ballSize = 11;
    ...
    public BreakoutPanel(int width, int height) {
        ...
        paddle = new Paddle((panelWidth / 2) - 64, panelHeight - 100, 16, 128, 16);
        ballIsDead = true;
    ...
    private void update() {
        ...
        if (keyHandler.isLeftPressed() || keyHandler.isRightPressed()) {
            if (keyHandler.isLeftPressed()) {
                ...
                if (paddle.getPaddleX() + paddle.getPaddleWidth() + paddle.getPaddleSpeed() < panelWidth) {
                    paddle.moveRight();
                }
            }
        }
            ...
            for (Brick brick : bricks) {
                if (!brick.isBroken()) {
                    ...
                    if (ballX >= brickX && ballX <= brickMaxX
                        ...
                        // for now, just flip the ball's direction
                        // on a brick break
